...
private:
  ...
  display_rows get_row_display() {
    ...
    int depth = 0;
    for (;;) {
      // Max-depth Nodes are always a leaf or nullptr
...
public:
  ...
  void iterativeInsert(T key) {
    ...
    std::shared_ptr<Node<T>>
        r; // this var keeps the correct location for node to be inserted
  ...
  size_t updateHeight(std::shared_ptr<Node<T>> node) {
    if (node != nullptr) {
      node->height =
          1 + std::max(updateHeight(node->left), updateHeight(node->right));
      return node->height;
    } else
      return 0;
  }
  ...
  size_t getBalanceFactor(std::shared_ptr<Node<T>> N) {
    ...
    std::cout << "node:" << N->value << " left height: " << getHeight(N->left)
              << " right height: " << getHeight(N->right) << std::endl;
  ...
  */
  void printGivenLevel(std::shared_ptr<Node<T>> root, int level) {
    if (root == nullptr)
      return;
    if (level == 1) {
      std::cout << "value: " << root->value << " height: " << root->height
                << " balance:" << getBalanceFactor(root) << std::endl;
    }
  ...
  void insert(T key) { iterativeInsert(key); }
  ...
  void createTree() {
    std::cout << "Creating a binary tree based on level order traverse, enter "
                 "-1 if a node doesn't have left(right) child"
              << std::endl;
...
int main() {
  ...
  */
  ...
  std::shared_ptr<BinarySearchTree<int>> tree =
      std::make_shared<BinarySearchTree<int>>(bst);
