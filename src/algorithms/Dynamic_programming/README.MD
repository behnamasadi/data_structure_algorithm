...
`Cost(1,{2,3,4})=min{dist<sub>1K<sub> +Cost(K,{2,3,4}-{K} )  }`  
or in general form:

`Cost(i,S)=min{dist<sub>iK<sub> +Cost(K,S-{K} )  }` 
...
```
                           ...
                           1   1  1   1   1   1 
```
If we solve the problem from botton (from most left hand side node):
```
Cost(3,{∅})=6  
Cost(4,{∅})=8  
Cost(2,{∅})=5  
