...
template <typename T> typename T::iterator nodeListMinElement(T &m) {
  return min_element(
      m.begin(), m.end(),
...
bool jobcomparison(job &lhs, job &rhs) { return lhs.profit > rhs.profit; }

void printJobList(std::vector<job> &jobsList) {
  for (std::size_t i = 0; i < jobsList.size(); i++) {
    std::cout << "job id :" << jobsList[i].id
              << " job profit : " << jobsList[i].profit
              << " job deadline: " << jobsList[i].deadline << std::endl;
  }
}

void JobSequenceWithDeadlines(std::vector<job> &jobsList,
                              std::vector<job> &optimaljobsList) {
  ...
  for (size_t i = 0; i < jobsList.size(); i++) {
    for (int j = std::min(jobsList.size() - 1, jobsList[i].deadline); j >= 0;
         j--) {
      if (!occupied[j]) {
        occupied[j] = true;
...
//	{
//		std::vector<std::shared_ptr<node>> waypoints =
// queue.top().waypoints; 		for (size_t i = 0; i < waypoints.size();
// i++)
//		{
//			std::cout << waypoints[i]->id << " ";
//		}
...
void dijkstraPathsFinder(std::shared_ptr<node> start,
...
int main() {
  {
    // std::vector < job > jobsList = { {"3",10,3}, {"1",20,2},{"4",5,3}
    // ,{"2",15,2}, {"5",1,3} };
    std::vector<JobSequence::job> jobsList = {
...

  {
    ...
    for (std::map<std::string, int>::iterator it = charFrequency.begin();
         it != charFrequency.end(); it++) {
    ...
    while (nodes.size() > 2) {
      std::vector<Huffman::node>::iterator firstMinPtr =
          Huffman::nodeListMinElement(nodes);
      Huffman::node firstMin = *(firstMinPtr);
      nodes.erase(firstMinPtr);

      std::vector<Huffman::node>::iterator secondMinPtr =
          Huffman::nodeListMinElement(nodes);
