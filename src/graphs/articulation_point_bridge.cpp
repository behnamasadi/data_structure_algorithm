...
class AdjacencyList
{
    ...
    AdjacencyList(size_t m)
    {
        data=std::vector(m,std::vector<T>());
    }
...
template
<typename T>
void dfs(size_t at, int parent,AdjacencyList<T> graphadjacencyList,std::vector<Edge<T>> &bridges)
{
    visited[at]=true;
    lows[at]=ids[at]=id++;
    ...
    {
        std::cout<<"visiting neighbours of "<<at <<", neighbour id:"<<neighbour <<std::endl;
        ...
        {
            std::cout<<"node id: " <<neighbour << " has been already visited"<<std::endl;
...
int main()
{
    ...
    {
        std::cout<< "Edge From: "<<edge.from<<" To " <<edge.to<<" is a brdige" <<std::endl;
    }

    for(size_t i =0;i<lows.size();i++)
    {
        std::cout<<"node id: " <<ids[i] << " lowest link value: "<< lows[i]<<std::endl;
    }
