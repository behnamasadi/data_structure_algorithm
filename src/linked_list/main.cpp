...
template <typename T> void printSTDList(std::list<T> &list) {
  for (typename std::list<T>::iterator it = list.begin(); it != list.end();
       it++) {
    std::cout << *it << "->";
  }
  std::cout << "nullptr" << std::endl;
...
template <typename T> void linkedListmiddleNode(std::list<T> &list) {
...
template <typename T, typename L> struct polynominalNode {
  L exponent;
  T coefficient;
};
...
void polynominalRepresentationUsingLinkedList() {
  ...
  double x = 5;
  double sum = 0;
  for (std::list<polynominalNode<double, int>>::iterator it =
           polynominalTerms.begin();
       it != polynominalTerms.end(); it++) {
