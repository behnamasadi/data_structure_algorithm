...
Available_Workshops* initialize(int * start_time,int * duration, int n)
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        WorkshopList[i].end_time= WorkshopList[i].duration+WorkshopList[i].start_time;
...
bool overlapChecker(std::vector<Workshop> &listofWorkshops,Workshop &candidate)
{
    int start, end;
...
int CalculateMaxWorkshops(Available_Workshops * ptr)
{
    ...
    for(int i=0;i<ptr->n;i++)
    {
        ...
        listofWorkshops.push_back(mustIncluded);
         ...
         max_workshop_to_attend=std::max(int(listofWorkshops.size()),max_workshop_to_attend);
    }
